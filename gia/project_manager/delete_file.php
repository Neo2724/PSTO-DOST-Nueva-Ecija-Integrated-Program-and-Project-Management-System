...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    ...
    // Fetch file name
    $stmt = $conn->prepare("SELECT file_name FROM gia_files WHERE project_id = ? AND LOWER(document_name) = LOWER(?)");
    ...
    if ($row = $result->fetch_assoc()) {
        ...
        $deleteStmt = $conn->prepare("DELETE FROM gia_files WHERE project_id = ? AND document_name = ?");
