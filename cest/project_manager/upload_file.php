...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $checkStmt = $conn->prepare("SELECT id FROM files WHERE project_id = ? AND document_name = ?");
    ...
    if (move_uploaded_file($file['tmp_name'], $targetPath)) {
        ...
        $stmt = $conn->prepare("INSERT INTO files (project_id, document_name, file_name, uploaded) VALUES (?, ?, ?, 1)");
